                                                    ...
                                                    harp_array data)
{
    ...
    {
        harp_set_error(HARP_ERROR_PRODUCT, "variable %s has %ld elements (expected %ld or %ld)", path,
...
static int read_variable_string(void *user_data, const char *path, long index, long num_elements, harp_array data)
{
    ...
    {
        harp_set_error(HARP_ERROR_PRODUCT, "first dimension of variable %s has %ld elements (expected %ld)", path,
                       dim[0], num_elements);
        return -1;
    }
    if (num_dims > 2)
    {
        harp_set_error(HARP_ERROR_PRODUCT, "variable %s has %d dimensions (expected <= 2)", num_dims);
        return -1;
    }
    else if (num_dims == 2)
    {
...
static int read_altitude_bounds(void *user_data, harp_array data)
{
    ...
    if (info->swap_alt_bounds)
    {
        ...
        for (i = 0; i < info->num_time; i++)
        {
            ...
            if (harp_array_transpose(harp_type_double, 2, dimension, sub_array) != 0)
            {
                return -1;
            }
        }
...
static int read_vmr_offaxis_cov_random(void *user_data, harp_array data)
{
    ingest_info *info = (ingest_info *)user_data;
    char path[MAX_PATH_LENGTH];
...
static int read_vmr_offaxis_cov_systematic(void *user_data, harp_array data)
{
    ingest_info *info = (ingest_info *)user_data;
    char path[MAX_PATH_LENGTH];
...
static int read_vmr_zenith_cov_random(void *user_data, harp_array data)
{
    ingest_info *info = (ingest_info *)user_data;
    char path[MAX_PATH_LENGTH];
...
static int read_vmr_zenith_cov_systematic(void *user_data, harp_array data)
{
    ingest_info *info = (ingest_info *)user_data;
    char path[MAX_PATH_LENGTH];
...
static int read_aerosol_extinction_coefficient_cov_random(void *user_data, harp_array data)
{
...
static int read_aerosol_extinction_coefficient_cov_systematic(void *user_data, harp_array data)
{
                                  ...
                                  harp_product_definition **definition)
{
    ...
    {
        expected_type = uvvis_doas_aerosol;
    }
    else if (length == 34)
    {
    ...
    if (expected_type == uvvis_doas_aerosol)
    {
...
static int get_dimensions(ingest_info *info)
{
    ...
    if (info->template_type == uvvis_doas_aerosol)
    {
        ...
        if (info->num_spectral > 1)
        {
            ...
            if (values[1] < values[0])
            {
                harp_set_error(HARP_ERROR_PRODUCT, "spectral dimension should use a wavelength ascending ordering");
                return -1;
            }
        }
    }
                          ...
                          const harp_ingestion_options *options, harp_product_definition **definition, void **user_data)
{
    ...
    switch (info->template_type)
    {
        case uvvis_doas_directsun:
            ...
            info->product_version = info->definition->name[36] - '0';
            break;
        case uvvis_doas_offaxis:
            info->gas = get_gas_from_string(&info->definition->name[36]);
            info->product_version = info->definition->name[34] - '0';
            break;
        case uvvis_doas_zenith:
            info->gas = get_gas_from_string(&info->definition->name[35]);
            info->product_version = info->definition->name[33] - '0';
            break;
        case uvvis_doas_aerosol:
            info->gas = -1;
            info->product_version = info->definition->name[38] - '0';
            break;
    }
                                   ...
                                   uvvis_doas_type template_type, int version)
{
    ...
    if (template_type == uvvis_doas_aerosol)
    {
        /* wavelength */
    ...
    else if (template_type == uvvis_doas_offaxis)
    {
        ...
        variable_definition = harp_ingestion_register_variable_full_read
            (product_definition, gas_var_name, harp_type_double, 2, dimension_type, NULL, gas_description, "ppv", NULL,
             read_vmr_offaxis);
        harp_variable_definition_add_mapping(variable_definition, NULL, NULL, gas_mapping_path, NULL);
        ...
        variable_definition = harp_ingestion_register_variable_full_read
            (product_definition, gas_var_name, harp_type_double, 3, dimension_type, NULL, gas_description, "ppv", NULL,
             read_vmr_offaxis_cov_random);
        harp_variable_definition_add_mapping(variable_definition, NULL, NULL, gas_mapping_path, NULL);
        ...
        variable_definition = harp_ingestion_register_variable_full_read
            (product_definition, gas_var_name, harp_type_double, 3, dimension_type, NULL, gas_description, "ppv", NULL,
             read_vmr_offaxis_cov_systematic);
        harp_variable_definition_add_mapping(variable_definition, NULL, NULL, gas_mapping_path, NULL);
    ...
    else if (template_type == uvvis_doas_zenith)
    {
        ...
        variable_definition = harp_ingestion_register_variable_full_read
            (product_definition, gas_var_name, harp_type_double, 2, dimension_type, NULL, gas_description, "ppv",
        ...
        variable_definition = harp_ingestion_register_variable_full_read
            (product_definition, gas_var_name, harp_type_double, 3, dimension_type, NULL, gas_description, "ppv",
    ...
    else if (template_type == uvvis_doas_aerosol)
    {
        ...
        /* aerosol_extinction_coefficient_cov_random */
        variable_definition = harp_ingestion_register_variable_full_read
            (product_definition, "aerosol_extinction_coefficient_cov_random", harp_type_double, 4, dimension_type, NULL,
             "random covariance of the aerosol extinction coefficient", "km^-2", NULL,
             read_aerosol_extinction_coefficient_cov_random);
        mapping_path = "/AEROSOL.EXTINCTION.COEFFICIENT_SCATTER.SOLAR.OFFAXIS_UNCERTAINTY.RANDOM.COVARIANCE";
        harp_variable_definition_add_mapping(variable_definition, NULL, NULL, mapping_path, NULL);

        /* aerosol_extinction_coefficient_cov_systematic */
        variable_definition = harp_ingestion_register_variable_full_read
            (product_definition, "aerosol_extinction_coefficient_cov_systematic", harp_type_double, 4, dimension_type,
             NULL, "systematic covariance of the aerosol extinction coefficient", "km^-2", NULL,
             read_aerosol_extinction_coefficient_cov_systematic);
...
int harp_ingestion_module_geoms_uvvis_doas_init()
{
    ...
    init_product_definition(module, -1, uvvis_doas_aerosol, 4);
    init_product_definition(module, -1, uvvis_doas_aerosol, 6);
