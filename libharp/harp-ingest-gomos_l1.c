...
static int get_scalar_value(ingest_info *info, const char *datasetname, const char *fieldname, variable_type dataset_type, double *double_data)
{
    coda_cursor cursor;
...
static int get_main_data(ingest_info *info, const char *datasetname, const char *fieldname, variable_type var_type, double *double_data_array)
{
    coda_cursor cursor;
    double *double_data;
    long l;
...
static int get_wavelength_data(ingest_info *info, const char *datasetname, const char *fieldname, double *double_data_array)
{
    coda_cursor cursor;
...
static int get_spectral_data(ingest_info *info, const char *datasetname, const char *fieldname, int upper_lower_index, double *double_data_array)
{
    coda_cursor cursor;
...
static int spectral_conversion_factor(ingest_info *info, char *datasetname, char *num_fieldname, char *wavelength_fieldname, char *factor_fieldname, double wavelength, double *conversion_factor)
{
    ...
    if (wavelengths_in_table == NULL)
    {
        ...
        if (coda_cursor_read_double_partial_array(&cursor, 0L, (long)nr_wavelengths_in_table, wavelengths_in_table) != 0)
        {
            harp_set_error(HARP_ERROR_CODA, NULL);
            return -1;
        }
        ...
        if (coda_cursor_read_double_partial_array(&cursor, 0L, (long)nr_wavelengths_in_table, conversion_factors_in_table) != 0)
        {
            harp_set_error(HARP_ERROR_CODA, NULL);
            return -1;
        }
    ...
    else
    {
        ...
        {
            if ((wavelength > wavelengths_in_table[i - 1]) &&
                (wavelength < wavelengths_in_table[i]))
            {
                *conversion_factor =
                    conversion_factors_in_table[i - 1] +
                    ((conversion_factors_in_table[i] - conversion_factors_in_table[i - 1]) *
                     ((wavelength - wavelengths_in_table[i - 1]) / (wavelengths_in_table[i] - wavelengths_in_table[i - 1])));
            }
        }
    }
...
{
    return get_main_data((ingest_info *)user_data, "lim_ads", "tangent_long", USE_UPPER_FLAG_AS_INDEX, data.double_data);
}
...
static int read_lim_altitude(void *user_data, harp_array data)
{
    ...
    retval = get_main_data((ingest_info *)user_data, "lim_ads", "tangent_alt", USE_UPPER_FLAG_AS_INDEX, data.double_data);
...
static int read_lim_spectral_photon_radiance(void *user_data, harp_array data)
{
    ...
    if (info->corrected)
    {
        retval = get_spectral_data(info, "lim_mds", "up_low_back_corr", info->upper ? 0 : info->num_wavelengths, background_code_values);
    }
    else
    {
        retval = get_spectral_data(info, "lim_mds", "up_low_back_no_corr", info->upper ? 0 : info->num_wavelengths, background_code_values);
    }
    if (retval != 0)
    {
    ...
    for (profile_nr = 0; profile_nr < info->elements_per_profile; profile_nr++)
    {
        for (wavelength_nr = 0; wavelength_nr < info->num_wavelengths; wavelength_nr++)
        {
            ...
            background_in_electrons = background_offsets[profile_nr] + *(background_code_values + profile_nr * info->num_wavelengths + wavelength_nr) / background_gains[profile_nr];
            ...
            if (spectral_conversion_factor(info, "lim_occultation_data", "size_rad_sens_curve_limb", "abs_rad_sens_curve_limb", "rad_sens_curve_limb", wavelength, &conversion_factor) == 0)
            {
...
static int read_lim_spectral_photon_radiance_error(void *user_data, harp_array data)
{
    ...
    info = (ingest_info *)user_data;
    return get_spectral_data(info, "lim_mds", "err_up_low_back_corr", info->upper ? 0 : info->num_wavelengths, data.double_data);
}
...
static int lim_ingestion_init(const harp_ingestion_module *module, coda_product *product, const harp_ingestion_options *options, harp_product_definition **definition, void **user_data)
{
    int format_version;
    ingest_info *info;
    const char *cp;
...
static void register_limb_product(void)
{
    ...
    module = harp_ingestion_register_module_coda("GOMOS_LIM_L1", "GOMOS", "ENVISAT_GOMOS", "GOM_LIM_1P", description, lim_ingestion_init, ingestion_done);

    harp_ingestion_register_option(module, "spectra", "retrieve the upper or lower background spectra; by default the upper spectra are retrieved; option values are 'upper' and 'lower'", 2, upper_lower_options);
    harp_ingestion_register_option(module, "corrected", "retrieve the corrected or uncorrected background spectra; by default the corrected spectra are retrieved; option values are 'true' and 'false'", 2, true_false_options);
    ...
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "datetime", harp_type_double, 1, dimension_type, NULL, description, "seconds since 2000-01-01", NULL, read_lim_datetime);
    path = "/lim_mds/dsr_time";
    harp_variable_definition_add_mapping(variable_definition, NULL, NULL, path, NULL);

    /* latitude_of_the_measurement */
    description = "The latitude of the apparent tangent point";
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "latitude", harp_type_double, 1, dimension_type, NULL, description, "degree_north", NULL, read_lim_latitude);
    ...
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "longitude", harp_type_double, 1, dimension_type, NULL, description, "degree_east", NULL, read_lim_longitude);
    ...
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "altitude", harp_type_double, 1, dimension_type, NULL, description, "km", NULL, read_lim_altitude);
    path = "/lim_ads/tangent_alt[]";
    harp_variable_definition_add_mapping(variable_definition, NULL, NULL, path, "The value is divided by 1000 to convert it to km");
    ...
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "spectral_photon_radiance", harp_type_double, 2, dimension_type, NULL, description, "ph/s/cm^-2/nm/sr", NULL, read_lim_spectral_photon_radiance);
    ...
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "spectral_photon_radiance_error", harp_type_double, 2, dimension_type, NULL, description, "%", NULL, read_lim_spectral_photon_radiance_error);
    ...
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "wavelength", harp_type_double, 1, &(dimension_type[1]), NULL, description, "nm", NULL, read_lim_wavelength);
    ...
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "datetime_length", harp_type_double, 0, dimension_type, NULL, description, "s", NULL, read_lim_integration_time);
    path = "/sph/samp_duration";
    harp_variable_definition_add_mapping(variable_definition, NULL, NULL, path, NULL);

    /* sensor_latitude */
    description = "The latitude of the satellite";
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "sensor_latitude", harp_type_double, 1, dimension_type, NULL, description, "degree_north", NULL, read_lim_sensor_latitude);
    harp_variable_definition_set_valid_range_double(variable_definition, -90.0, 90.0);
    path = "/lim_ads/lat[]";
    harp_variable_definition_add_mapping(variable_definition, NULL, NULL, path, NULL);
    ...
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "sensor_longitude", harp_type_double, 1, dimension_type, NULL, description, "degree_east", NULL, read_lim_sensor_longitude);
    harp_variable_definition_set_valid_range_double(variable_definition, -180.0, 180.0);
    path = "/lim_ads/longit[]";
    harp_variable_definition_add_mapping(variable_definition, NULL, NULL, path, NULL);
    ...
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "sensor_altitude", harp_type_double, 1, dimension_type, NULL, description, "km", NULL, read_lim_sensor_altitude);
    path = "/lim_ads/alt[]";
    harp_variable_definition_add_mapping(variable_definition, NULL, NULL, path, "The value is divided by 1000 to convert it to km");

    /* illumination_condition */
    description = "The illumination condition for each profile: 'dark', 'bright', 'twilight', 'straylight' or 'twilight/straylight'";
    variable_definition = harp_ingestion_register_variable_sample_read(product_definition, "flag_illumination_condition", harp_type_string, 0, dimension_type, NULL, description, NULL, NULL, read_lim_illumination_condition);
...
{
    return get_main_data((ingest_info *)user_data, "tra_geolocation", "tangent_lat", USE_ARRAY_INDEX_1, data.double_data);
}

static int read_tra_longitude(void *user_data, harp_array data)
{
    return get_main_data((ingest_info *)user_data, "tra_geolocation", "tangent_long", USE_ARRAY_INDEX_1, data.double_data);
}
...
static int read_tra_altitude(void *user_data, harp_array data)
{
    ...
    retval = get_main_data((ingest_info *)user_data, "tra_geolocation", "tangent_alt", USE_ARRAY_INDEX_1, data.double_data);
    /* Convert read altitude from meters to kilometers */
    for (l = 0; l < ((ingest_info *)user_data)->elements_per_profile; l++)
    {
        data.double_data[l] = data.double_data[l] / 1000.0;
...
static int read_tra_spectral_photon_irradiance(void *user_data, harp_array data)
{
    ...
    retval = get_main_data((ingest_info *)user_data, "tra_auxiliary_data", "off_back", IS_NO_ARRAY, transmission_offsets);
    if (retval != 0)
    {
    ...
    retval = get_main_data((ingest_info *)user_data, "tra_auxiliary_data", "gain_back", IS_NO_ARRAY, transmission_gains);
    if (retval != 0)
    {
    ...
    for (profile_nr = 0; profile_nr < info->elements_per_profile; profile_nr++)
    {
        for (wavelength_nr = 0; wavelength_nr < info->num_wavelengths; wavelength_nr++)
        {
            ...
            transmission_in_electrons = transmission_offsets[profile_nr] + *(transmission_code_values + profile_nr * info->num_wavelengths + wavelength_nr) / transmission_gains[profile_nr];
            wavelength = info->wavelengths[wavelength_nr];
            /* Convert the value in electrons to a physical unit according */
            /* to section 10.4.1.7.2 in the ENVISAT-GOMOS product          */
            /* specifications (ESA Doc Ref: PO-RS-MDA-GS-2009).            */
            if (spectral_conversion_factor(info, "tra_occultation_data", "size_rad_sens_curve_star", "abs_rad_sens_curve_star", "rad_sens_curve_star", wavelength, &conversion_factor) == 0)
            {
...
static int tra_ingestion_init(const harp_ingestion_module *module, coda_product *product, const harp_ingestion_options *options, harp_product_definition **definition, void **user_data)
{
    int format_version;
    ingest_info *info;
...
static void register_tra_product(void)
{
    ...
    description = "GOMOS Level 1";
    module = harp_ingestion_register_module_coda("GOMOS_TRA_L1", "GOMOS", "ENVISAT_GOMOS", "GOM_TRA_1P", description, tra_ingestion_init, ingestion_done);
    ...
    description = "GOMOS Level 1 products only contain a single profile; all measured transmission data will be provided "
        "in order from high altitude to low altitude in the profile";
    harp_product_definition_add_mapping(product_definition, description, NULL);
    ...
    /* time_of_the_measurement */
    description = "The time of the measurement";
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "datetime", harp_type_double, 1, dimension_type, NULL, description, "seconds since 2000-01-01", NULL, read_tra_datetime);
    path = "/tra_transmission/dsr_time";
    harp_variable_definition_add_mapping(variable_definition, NULL, NULL, path, NULL);

    /* latitude_of_the_measurement */
    description = "The latitude of the apparent tangent point";
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "latitude", harp_type_double, 1, dimension_type, NULL, description, "degree_north", NULL, read_tra_latitude);
    harp_variable_definition_set_valid_range_double(variable_definition, -90.0, 90.0);
    ...
    /* longitude_of_the_measurement */
    description = "The longitude of the apparent tangent point";
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "longitude", harp_type_double, 1, dimension_type, NULL, description, "degree_east", NULL, read_tra_longitude);
    harp_variable_definition_set_valid_range_double(variable_definition, -180.0, 180.0);
    ...
    /* altitude_of_the_measurement */
    description = "The altitude of the apparent tangent point in km";
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "altitude", harp_type_double, 1, dimension_type, NULL, description, "km", NULL, read_tra_altitude);
    ...
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "spectral_photon_irradiance", harp_type_double, 2, dimension_type, NULL, description, "ph/s/cm^-2/nm", NULL, read_tra_spectral_photon_irradiance);
    ...
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "spectral_photon_irradiance_error", harp_type_double, 2, dimension_type, NULL, description, "ph/s/cm^-2/nm", NULL, read_tra_spectral_photon_irradiance_error);
    path = "/tra_transmission[]/cov[]";
    harp_variable_definition_add_mapping(variable_definition, NULL, NULL, path, NULL);

    /* wavelength_of_each_spectrum_measurement */
    description = "The nominal wavelength assignment for each of the detector pixels";
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "wavelength", harp_type_double, 1, &(dimension_type[1]), NULL, description, "nm", NULL, read_tra_wavelength);
    path = "/tra_nom_wav_assignment[]/nom_wl[]";
    harp_variable_definition_add_mapping(variable_definition, NULL, NULL, path, "Will be set to nm");

    /* integration time */
    description = "The integration time for a readout (in seconds)";
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "datetime_length", harp_type_double, 0, dimension_type, NULL, description, "s", NULL, read_tra_integration_time);
    path = "/sph/samp_duration";
    harp_variable_definition_add_mapping(variable_definition, NULL, NULL, path, NULL);

    /* sensor_latitude */
    description = "The latitude of the satellite position at half-measurement";
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "sensor_latitude", harp_type_double, 1, dimension_type, NULL, description, "degree_north", NULL, read_tra_sensor_latitude);
    harp_variable_definition_set_valid_range_double(variable_definition, -90.0, 90.0);
    ...
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "sensor_longitude", harp_type_double, 1, dimension_type, NULL, description, "degree_east", NULL, read_tra_sensor_longitude);
    harp_variable_definition_set_valid_range_double(variable_definition, -180.0, 180.0);
    ...
    variable_definition = harp_ingestion_register_variable_full_read(product_definition, "sensor_altitude", harp_type_double, 1, dimension_type, NULL, description, "km", NULL, read_tra_sensor_altitude);
    path = "/tra_geolocation/alt[1]";
    harp_variable_definition_add_mapping(variable_definition, NULL, NULL, path, "The value is divided by 1000 to convert it to km");

    /* illumination_condition */
    description = "The illumination condition for each profile: 'dark', 'bright', 'twilight', 'straylight' or 'twilight/straylight'";
    variable_definition = harp_ingestion_register_variable_sample_read(product_definition, "flag_illumination_condition", harp_type_string, 0, dimension_type, NULL, description, NULL, NULL, read_tra_illumination_condition);
