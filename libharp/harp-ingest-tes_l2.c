                         ...
                         harp_array data)
{
    ...
    {
        harp_set_error(HARP_ERROR_PRODUCT, "product error detected in TES L2 product (variable %s has %d dimensions, "
                       "expected %d)", name, num_coda_dimensions, num_dimensions);
        return -1;
    }
    if (dimension_0 != coda_dimension[0])
    {
        harp_set_error(HARP_ERROR_PRODUCT, "product error detected in TES L2 product (first dimension for variable %s "
    ...
    if (num_dimensions > 1)
    {
        ...
        {
            harp_set_error(HARP_ERROR_PRODUCT, "product error detected in TES L2 product (second dimension for "
